  ...
  class SearchComicsService
    ...
    def perform(page, page_size, character = nil)
      ...
      if character.present?
        characters = Rails.cache.fetch(characters_cache_key(character), expires_in: 12.hours) do
          characters_response = ApiService.new.get_characters_by_name(character)
          return nil unless characters_response.present?

          characters_response.body["data"]["results"].map { |character| character["id"] }
        end
      end

      data = Rails.cache.fetch(comics_cache_key(page, page_size, character), expires_in: 12.hours) do
